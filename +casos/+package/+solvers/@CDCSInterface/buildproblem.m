...
%
% CDCS solves semidefinite problems in the form
%
%   (primal) min c'*x st. A*x = b and x in K
%
%   (dual)   max b'*y st. A'*y + z = c and z in K*
%
% where A, b and c are the problem data. The cone K is a cone composed of 
%   - free variables (K.f), 
%   - nonnegative orthant (K.l), 
%   - Lorentz cone (K.q), and  
%   - cone of positive semidefinite matrices (K.s),
% and K* is the dual cone of K.
%
% For more details on the CDCS: https://github.com/oxfordcontrol/CDCS
%
% WARNING: CDCS does not support the rotated Lorenz cone (K.r).
...
% reorder decision variables (z,s) to (l,q,s)
% current order: (z,s) = [zl zc sua sla sca sux]
Iz = [ones(1,Nx.l) 2*ones(1,sum(Nx.q))  4*ones(1,sum(Nx.s.^2))];
Ia = [ones(1,2*Na.l) 2*ones(1,sum(Na.q))  4*ones(1,sum(Na.s.^2))];
Ix =  ones(1,Nx.l);

% new order: [zl sua sla sux | zc_q sca_q zc_s sca_s] 
% (see https://de.mathworks.com/help/matlab/ref/sort.html#bt8nojg-1-I)
[~,idx] = sort([Iz Ia Ix]);

% return CDCS structures A,b,c
obj.fhan = casadi.Function('f',struct2cell(obj.args_in),{A(:,idx) b c(idx)},fieldnames(obj.args_in),{'A' 'b' 'c'});

% parse CDCS solution (X,Y) into (x,cost,lam_a,lam_x)
X = casadi.MX.sym('x',size(c));
Y = casadi.MX.sym('y',size(b));
S = c - A'*Y;

% primal decision variables in the NEW order
Xc = mat2cell(X,[Nx.l Na.l Na.l Nx.l sum(Nx.q) sum(Na.q) sum(Nx.s.^2) sum(Na.s.^2)],1);
x = [Xc{1}; Xc{5}; Xc{7}] + zb;
